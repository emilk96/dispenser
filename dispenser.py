...
class SampleApp(tk.Tk):
    ...
    def demo(self):
        ...
        while time.time() < (now+5):
            ...
            if scanned_item == ['S','1','4','9','4','3','7','5','7']:
                self.movemotor_sample_backward()
                self.movemotor_solution_forward()
                var3.set("No Covid... just ugly :D") 
                self.update_idletasks() 
                break
            elif len(scanned_item) == 9:
                self.movemotor_sample_backward()
                self.movemotor_solution_forward()
                break
            else:
    ...
    #     motor_sample.run(1600, True)     # run motor 6400 steps clowckwise
    #     sleep(0.5)
    #     motor_sample.run(1600, False)    # run motor 6400 steps counterclockwise
    ...
    def movemotor_solution_forward(event):
        ...
        motor_solution.run(1600, True)     # run motor 6400 steps clowckwise
        motor_solution.enable(False)       # disable stepper driver
    
    def movemotor_solution_backward(event):
        # Enable motor and run motor script        
        motor_solution.enable(True)        # enables stepper driver
        motor_solution.run(1600, False)    # run motor 6400 steps counterclockwise
        motor_solution.enable(False)       # disable stepper driver
...
class PageOne(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button2 = tk.Button(self, text="Move Back Mechanism", width=20, height=5,bg='#4974a5', fg='white',
                            command=lambda: controller.movemotor_sample_backward()) #Probably needs an own function for open and closing
...
class PageFive(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, textvariable = var3, font=controller.title_font)
        label.place(x=x_secondcolumn, y=350)
