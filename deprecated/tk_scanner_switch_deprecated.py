...
## GPIO setup
## define motor objec: MotorName = StepperMotor(enablePin, stepPin, dirPin)
motor = StepperMotor(23, 24, 25)
...
class SampleApp(tk.Tk):
    ...
    def run_scanner(self):
        ...
        while running:
            ...
            if scanned_item == ['S','1','4','9','4','3','7','5','7']:
                ...
                break
            if scanned_item == ['S','1','1','9','4','7','8','7','6']:
                var.set("--") 
                var2.set("Please activate scanner!") 
                break
            else:
    ...
    def movemotor(event):
        ...
        motor.run(100, True)     # run motor 6400 steps clowckwise
        sleep(0.5)
        motor.run(64, False)    # run motor 6400 steps counterclockwise
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button4 = tk.Button(self, text="Motor Controls",
...
#include legipage in self.frames = {}
    def __init__(self, parent, controller):
        ...
        self.controller = controller
        self['bg']='#4974a5'
        #imageBg = tk.PhotoImage(file='Collector_2_deny.png')
        #self.image = imageBg
        #label_bg = tk.Label(self, image=imageBg)
        #label_bg.place(x=0, y=0)
        #label = tk.Label(self, text="This is page 1", font=controller.title_font,background='#4974a5')
        #label.pack(side="top", fill="x", pady=10)
        ...
        label3 = tk.Label(self, textvariable = var2, font=tkfont.Font(family='Helvetica', size=10),background='#4974a5')
        label3.pack()
...
class MotorPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button1 = tk.Button(self, text="Move Motor 1",
                            command=lambda: controller.movemotor())
        button2 = tk.Button(self, text="Move Motor 2",
                            command=lambda: controller.movemotor())
        button3 = tk.Button(self, text="Move Motor 3",
                            command=lambda: controller.movemotor())
        button4 = tk.Button(self, text="Move Motor 4",
                            command=lambda: controller.movemotor())
        button5 = tk.Button(self, text="Move Motor 5",
                            command=lambda: controller.movemotor())
        button = tk.Button(self, text="Go to Hack Panel",
                           command=lambda: controller.show_frame("StartPage"))
        button1.pack()
